...
describe('Passkey login', () => {
  ...
  it('allows Alice to access the home page with her valid passkey', () => {
    ...
    cy.getByTest('api-greeting').should('contain.text', 'Hello from FarmersLeague API');
    cy.getByTest('match-teams').should('contain.text', 'Canada vs Mexico');
    cy.getByTest('no-access').should('not.exist');
  });
  ...
  it('denies access for an unknown passkey', () => {
    ...
    cy.getByTest('no-access').should('be.visible').and('contain.text', 'No access');
    cy.contains('h1', 'FarmersLeague').should('not.exist');
    cy.getByTest('api-greeting').should('not.exist');
  });
